...
const WebGLRenderer = forwardRef<HTMLDivElement, WebGLRendererProps>(({
  ...
  useEffect(() => {
    setFullscreenRef((ref as any).current);
  }, [ref, setFullscreenRef]);

  // Handle user data sending to Unity
  useEffect(() => {
    const handleSendUserToUnity = (userData: any) => {
  ...
  return (
    <div className="webgl-renderer">
      ...
      {/* Sign In Overlay */}
      {showSignInModal && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] w-full h-full flex justify-center items-center">
          <SignIn
            mode="button"
            label="Sign In"
            initialIsOpen={true}
      ...
      {/* Portal Manager removed - using plain div */}
      <div
        ref={ref as any}
      ...
      >
        ...
        <div
          {...fadeStyles}
          className="absolute z-[2] top-4 left-4 flex items-start gap-3 h-[50px]"
        >
          {settings.showDisruptiveLogo && (
            <img src={settings.urlDisruptiveLogo} className="w-[100px]" alt="Disruptive Logo" />
          )}
        </div>
        ...
        <div
          {...fadeStyles}
          className="absolute z-[2] bottom-4 right-4 flex items-start gap-3"
        >
