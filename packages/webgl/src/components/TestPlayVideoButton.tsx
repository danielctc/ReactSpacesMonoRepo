...
interface TestPlayVideoButtonProps {
  isEditMode?: boolean;
}

function TestPlayVideoButton({ isEditMode }: TestPlayVideoButtonProps) {
  ...
  const handlePlayVideo = async () => {
    try {
      ...
      if (video && !isEditMode) {
        ...
        (window as any).dispatchReactUnityEvent("PlayVideo", { gameObjectName: "TV2" });
      } else {
