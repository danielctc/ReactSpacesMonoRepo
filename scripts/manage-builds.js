...
function populateDistWithHistoricalBuilds() {
  ...
  fs.readdirSync(buildsDir).forEach((build) => {
    ...
    if (fs.statSync(buildPath).isDirectory()) {
      fs.readdirSync(buildPath).forEach((pkgName) => {
        // Skip excluded packages
        if (excludedPackages.includes(pkgName)) {
          return;
        }

        const assetsPath = path.join(buildPath, pkgName, 'assets');
        if (fs.existsSync(assetsPath) && fs.statSync(assetsPath).isDirectory()) {
          fs.readdirSync(assetsPath).forEach((file) => {
            if (file.endsWith('.js') || file.endsWith('.css')) {
              const fullPath = path.join(assetsPath, file);
              const targetDir = path.join(distDir, pkgName, 'assets');
...
function logScriptTags() {
  ...
  fs.readdirSync(distDir).forEach((pkgName) => {
    ...
    if (pkgName === 'testing' || excludedPackages.includes(pkgName)) {
      return;
    }
    const assetsPath = path.join(distDir, pkgName, 'assets');
    if (fs.existsSync(assetsPath) && fs.statSync(assetsPath).isDirectory()) {
      fs.readdirSync(assetsPath).forEach((file) => {
        if (file.endsWith('.css')) {
          const cssPath = `${baseURL}/${pkgName}/assets/${file}`;
          console.log(`\n${COLORS.CYAN}<!-- ${pkgName} CSS -->${COLORS.RESET}`);
          console.log(`<link rel="stylesheet" crossorigin href="${cssPath}">`);
        }
        if (file.endsWith('.js')) {
          const scriptPath = `${baseURL}/${pkgName}/assets/${file}`;
